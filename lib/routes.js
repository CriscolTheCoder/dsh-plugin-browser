...
/**
 * Curated alias map: catalog repo (normalized, lowercase) or entry name →
 * real package names that may be installed in the profile. Covers
 * monorepo/subdirectory packages and repository moves that the upstream
 * awesome-dsh-plugin registry has not mapped yet (e.g. dsh-deep-whale's
 * skin lives in a subpackage; dsh_workflow moved orgs).
 */
const KNOWN_ALIASES = {
  'small-tailqwq/dsh-deep-whale': ['@dsh-external/dsh-client-ui-skin-maid-atelier'],
  'dsh-external/dsh_workflow': ['@dsh-external/workflow'],
  'icetomoyo/dsh_workflow': ['@dsh-external/workflow'],
...
/** Real package names that would satisfy a catalog entry via the alias map. */
export function aliasPackages(entry) {
  const names = [];
  const repo = normalizeRepo(entry.url);
  if (repo !== null && KNOWN_ALIASES[repo]) names.push(...KNOWN_ALIASES[repo]);
  if (KNOWN_ALIASES[entry.name]) names.push(...KNOWN_ALIASES[entry.name]);
  return names;
}
...
function mergeEntry(entry, installed) {
  ...
  if (found === null) {
    for (const alias of aliasPackages(entry)) {
      if (installed.has(alias)) { found = installed.get(alias); viaAlias = true; break; }
    }
  }
  return {
    ...entry,
